...
- 📊 **账号级全局监控**: 
    ...
    - **NATS 集群多维拓扑与监控**:
        - **动态关系网络拓扑图**: 实时渲染集群核心节点、Routes 链路与 Leafnodes 叶子节点。使用物理力学仿真引擎 (`d3-force`) 防止重叠遮挡，链路按网络延迟 (RTT) 自动着色。结构未改变时刷新坐标完全持久稳定，杜绝漂移和旋转抖动。
        - **节点详情抽屉与配置引导**: 支持点击节点滑出侧边栏展示 CPU、内存、连接数、带宽吞吐及慢消费者指标。针对未开启 HTTP 监控的节点展示灰色置灰连线，并内置交互式 Alert 命令配置引导。
        - **节点性能面板**: 网格化展示集群中所有服务器性能（如 CPU、内存、Uptime、吞吐），同时支持将未监控的节点置灰展现并提供排查说明。
...
- 🔍 **微服务发现**:
    ...
    - **集成端点级 API 测试控制台（调试面板）**：微服务的每个端点（Endpoint）提供专属调试弹窗（Modal）。支持 JSON/YAML 语法有效性校验、一键格式化、超时设置、请求头注入、响应耗时计算和请求中止。
...
- [x] **NATS 集群多维拓扑与监控**: 可视化展示集群拓扑图，包括 Leafnodes（叶子节点）与集群路由的状态、延迟（RTT）和传输带宽。
- [x] **微服务调试面板**: 服务面板集成一键 Endpoint 接口在线调试控制台，提供 Payload 代码编辑器与格式校验、请求头自定义、超时中止和响应延迟耗时展示。
- [x] **Payload 编辑器语法高亮与格式化**: 在消息发布/请求面板集成富文本编辑器，支持 JSON/YAML 语法高亮、自动格式化与合法性验证。
